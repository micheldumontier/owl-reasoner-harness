#!/usr/bin/env bash
# elk via the shared OWLAPI driver (java/ReasonerCli.java), NOT via `robot reason`.
#
...
exec java -XX:ActiveProcessorCount=${JT} -XX:CompressedClassSpaceSize=256m -Xmx${XMX} -Dfile.encoding=UTF-8 -cp "$JAR:$DIR" ReasonerCli elk "$1" "$out"
